...
(function() {
  ...
  module.controller('GaWmsGetCapDirectiveController', function($scope,
      $translate, $window, gaWms) {
    var fileReader;
    ...
    $scope.$watch('getCap', function(val) {
      try {
        ...
        $window.console.error('WMS GetCap parsing failed: ', e);
      ...
      if (val && val.Service && val.Capability) {
        ...
        if (val.Capability.Layer) {
          var root = getChildLayers(val, val.Capability.Layer, $scope.map);
          if (root) {
    ...
    $scope.addLayerSelected = function() {
      var getCapLayer = $scope.options.layerSelected;
      if (getCapLayer) {
    ...
    var getChildLayers = function(getCap, layer, map) {
      ...
      if (layer.Layer) {
        ...
        for (var i = 0; i < layer.Layer.length; i++) {
          var l = getChildLayers(getCap, layer.Layer[i], map);
          if (!l) {
   ...
   */
  module.directive('gaWmsGetCap', function() {
    return {
      restrict: 'A',
      ...
      controller: 'GaWmsGetCapDirectiveController',
      link: function(scope) {}
    };
  });
})();
