      ...
      ['$scope', '$http', '$log', 'gaGlobalOptions', 'gaBrowserSniffer',
       function($scope, $http, $log, gaGlobalOptions, gaBrowserSniffer) {
         ...
         $scope.handleFileUrl = function() {
           ...
           if (url && url.length > 0 && /^http/i.test(url)) {
            ...
            .error(function(data, status, headers, config) {
               $scope.userMessage = 'Upload file failed !!!' +
                   ' Verify that your file exists';
               $scope.progress = 0;
               $log.error('Display KML failed');
             });

           } else {
             alert("You drop an invalid url: " + url); 
           }
         };
         ...
         $scope.handleReaderError = function(evt) {
           $scope.$apply(function() {
             $scope.userMessage = 'Read file failed !!!';
             $scope.progress = 0;
           });
         };
         ...
         $scope.displayFileContent = function() {
           ...
           try {
             ...
             $scope.userMessage = 'File parsed. ' +
             'Your data are now display on the map.';
             $scope.progress += 20;
             ...
             $scope.userMessage = 'Parsing failed for the following reason: ' +
             e.message;
             $scope.progress = 0;
