...
def runKmlTest(driver, target, is_top_browser):
    ...
    # Click on "Werkzeuge"
    driver.find_element_by_css_selector(
        "#toolsHeading > span.ng-scope").click()
    # Click on "KML Import"
    driver.find_element_by_xpath(
        "//div[@id='tools']/div/ul/li[2]/a/span").click()
    # Click on "URL"
    driver.find_element_by_link_text("URL").click()
    # Write URL of the chosen KML
    driver.find_element_by_css_selector("div > input[name=\"url\"]").send_keys(
        "http://opendata.utou.ch/urbanproto/geneva/geo/kml/Routes.kml")
    # Click on "KML Laden"
    driver.find_element_by_xpath(
        "//div[@id='import-kml-popup']/div[2]/div/div/div[2]/button").click()
    # Check if the KML was correctly parsed
    for i in range(60):
        try:
            if "Parsing erfolgreich" == driver.find_element_by_css_selector(
                    "div.ga-import-kml-result.ng-binding").text:
                break
        except:
            pass
        time.sleep(1)
    ...
    # Close popup
    driver.find_element_by_xpath(
        "//div[@id='import-kml-popup']/div/div[2]/button[3]").click()
    ...
    assert "Lignes" in driver.find_element_by_xpath(
        "//*[@id='selection']//*").text
